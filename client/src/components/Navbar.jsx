...
import { LayoutDashboard, LogOut, MapPin, Package, ShoppingBag, UserCircle } from 'lucide-react';
        ...
        <nav className="hidden items-center gap-2 text-sm font-semibold md:flex">
          {isAdmin ? (
            ...
            <>
              ...
              <NavLink to="/account" className={({ isActive }) => `inline-flex items-center gap-2 rounded-lg px-4 py-2 ${isActive ? 'bg-electric text-white' : 'text-muted hover:bg-slate-50 hover:text-ink'}`}><UserCircle size={16} /> Account</NavLink>
            </>
          )}
        </nav>
        <div className="flex items-center gap-2">
          {isAdmin ? (
            ...
            <>
              ...
              <NavLink to="/account" className={({ isActive }) => `rounded-lg border border-slate-200 p-2 ${isActive ? 'bg-electric text-white' : 'text-muted hover:border-ink'}`} title="Account"><UserCircle size={22} /></NavLink>
